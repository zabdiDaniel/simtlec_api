...
class TabletRegistrationScreen extends StatefulWidget {
  ...
  userData; // Agregado para recibir el RPE del usuario
...
class _TabletRegistrationScreenState extends State<TabletRegistrationScreen> {
  ...
  final _opcionesObservaciones = const ['Asignación inicial'];
  ...
  Future<void> _registrarTableta() async {
    ...
    try {
      ...
      // Registrar en historial solo si tiene trabajador asignado
      if (_trabajadorAsignado != null) {
        try {
          await TabletasApi.registrarHistorial(
            activo: _activoController.text,
            rpeTrabajador: _trabajadorAsignado!['rpe'],
            tipoAsignacion: _selectedObservacion ?? 'Asignación inicial',
          ...
          _showSnackBar('Error al registrar historial: $e', isError: true);
        }
      }
        ...
        if (_fotos[i] != null) {
          try {
            await TabletasApi.subirFoto(
              ...
              fotoIndex: i + 1,
              token: '', // Reemplaza con un token válido si es necesario
            );
          } catch (e) {
            todasSubidas = false;
            debugPrint('Error subiendo foto ${i + 1}: $e');
          }
        }
      }
              ...
              .map(
                (item) => DropdownMenuItem(
                  ...
                  child: Text(
                    ...
                    overflow:
                        TextOverflow
                            .ellipsis, // Trunca el texto con "..." si es demasiado largo
    ...
    return Column(
      children: [
        DropdownButtonFormField<String>(
          ...
          items:
              _opcionesObservaciones
                  .map(
                    (opcion) =>
                        DropdownMenuItem(value: opcion, child: Text(opcion)),
                  )
                  .toList(),
          onChanged: (value) => setState(() => _selectedObservacion = value),
